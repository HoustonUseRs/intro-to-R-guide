...
```{r}
airquality %>% summarise(mean(Ozone, na.rm=T))
```
...
Behind the scenes, R identifies the unique characters strings in the field, substitutes each one with a unique integer, and remembers the original strings as _levels_. 
...
```{r}
myname <- data.frame(letter=c("k","e","l","s","e","y"), stringsAsFactors=F)
```
