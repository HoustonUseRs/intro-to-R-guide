...
<img src="assets/create_new_file.png" style="width:200px">
...
Again, we use `mutate` to add another new column `temp_fac`, which is a factor version of `temp2`. Also, you will notice we have explicitly specified the levels in an order we would like (`low` and then `high`). This is confirmed in the `str` command.

Alright! Now let's see how `temp_fac` looks in our boxplot.
