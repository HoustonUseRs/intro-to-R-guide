...
keep a reasonable log of what we did (and possibly why).
...
We would go a little further, and bin the temperature variable into `high` and `low`, to enable comparison with ozone levels.
...
```{r}
aircomplete <- aircomplete %>% mutate(temp_fac = factor(temp2, levels = c("low", "high")))
```
