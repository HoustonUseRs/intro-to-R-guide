...
Additionally, `cor` and `cor.test` are two functions that can tell us about correlations between paired samples. The `cor` function will tell us the _correlation coefficient_ of two vectors. The `cor.test` function will allow us to test the strength of the association using one of several well-known correlation tests.
...
We can run these functions on our data by using `crickets$Chirpspersecond` as the y, and `crickets$Temperature` as the x like so:

```{r}
cor(x = crickets$Temperature, y = crickets$Chirpspersecond)
cor.test(x = crickets$Temperature, y = crickets$Chirpspersecond)
```

You can see that we've calculated the correlation coefficient and run a Pearson's product moment correlation coefficient.  We are pretty confident based on this info that the temperature outside is strongly correlated with the number of cricket chirps per second.
